...
const SESSION_ID       = 'sc-session-' + Date.now();
...
async function creaSessione() {
  try {
    ...
    console.log('[✅ Sessione creata]', SESSION_ID);

    // Visita homepage subito per inizializzare i cookie
    await fetchSC(SC_DOMAIN);
...
// ─── FLARESOLVERR: fetch con sessione ─────────────────────────────────────────
async function fetchSC(url) {
  ...
  console.log('[✅ FlareSolverr] ok, HTTP status:', data.solution.status);
...
async function cercaSuSC(query) {
  try {
    ...
    // Debug per vedere la struttura della risposta
    console.warn('[🔍 Debug HTML]', html.substring(0, 800));
...
// Crea sessione FlareSolverr dopo l'avvio del server
setTimeout(creaSessione, 2000);
