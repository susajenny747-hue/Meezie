...
async function cercaSuSC(query) {
  // Prova 1: /search con Inertia headers
  try {
    ...
    console.warn('[⚠️ Ricerca /search]', e.message);
  }

  // Prova 2: /api/search senza Inertia
  try {
    ...
    const risultati = data?.data || data || [];
    if (Array.isArray(risultati) && risultati.length > 0) {
      console.log(`[✅ Ricerca SC /api] ${risultati.length} risultati`);
      return risultati;
    }
  } catch (e) {
    console.warn('[⚠️ Ricerca /api/search]', e.message);
  }
