...
async function inizializzaSessione() {
  try {
    ...
    // Debug homepage subito dopo sessione
    await debugHomepage();
  } catch (e) {
...
async function debugHomepage() {
  try {
    console.log('[🔍 Debug] Fetch homepage...');
    const { data: html } = await axios.get(SC_DOMAIN, {
      headers: getHeaders(),
      timeout: 15000
    });
    const str = typeof html === 'string' ? html : JSON.stringify(html);
    const apiMatch    = str.match(/["'](\/api\/[^"'<>\s]{1,60})["']/g);
    const searchMatch = str.match(/search[^"'<>{}\s]{0,60}/gi);
    console.log('[🏠 API routes]'  , JSON.stringify(apiMatch?.slice(0, 15)));
    console.log('[🏠 Search refs]' , JSON.stringify(searchMatch?.slice(0, 10)));
    console.log('[🏠 HTML inizio]' , str.substring(0, 1500));
  } catch (e) {
    console.warn('[⚠️ Debug]', e.message);
  }
}
...
async function cercaSuSC(query) {
  const endpoints = [
    `${SC_DOMAIN}/api/search?q=${encodeURIComponent(query)}`,
    `${SC_DOMAIN}/search?q=${encodeURIComponent(query)}`,
    `${SC_DOMAIN}/?search=${encodeURIComponent(query)}`,
  ];
  ...
  for (const url of endpoints) {
    try {
      ...
      const { data } = await axios.get(url, {
        headers: getHeaders({ 'X-Inertia': 'true', 'X-Requested-With': 'XMLHttpRequest' }),
        timeout: 10000
      });

      // Se risposta JSON diretta
      if (typeof data === 'object') {
        const risultati = data?.data
                        || data?.titles
                        || data?.props?.titles?.data
                        || data?.props?.data
                        || [];
        if (Array.isArray(risultati) && risultati.length > 0) {
          console.log(`[✅ Trovati] ${risultati.length} da ${url}`);
          return risultati;
        }
        console.log('[🔍 JSON ricevuto]', JSON.stringify(data).substring(0, 300));
      }

      // Se HTML
      if (typeof data === 'string') {
        console.log('[🔍 HTML ricevuto]', data.substring(0, 300));
        const match = data.match(/data-page="([^"]+)"/)
                   || data.match(/data-page='([^']+)'/);
        if (match) {
          const json = JSON.parse(match[1].replace(/&quot;/g, '"').replace(/&#039;/g, "'"));
          const risultati = json?.props?.titles?.data || json?.props?.data || json?.data || [];
          if (Array.isArray(risultati) && risultati.length > 0) {
...
async function getVideoIdFilm(scTitoloId, slug) {
  try {
    ...
    if (typeof data === 'object') {
      return data?.props?.title?.videos?.[0]?.id || null;
    }
    const match = data.match(/data-page="([^"]+)"/) || data.match(/data-page='([^']+)'/);
    if (!match) return null;
    const json = JSON.parse(match[1].replace(/&quot;/g, '"').replace(/&#039;/g, "'"));
    return json?.props?.title?.videos?.[0]?.id || null;
  } catch (e) {
...
async function getVideoIdEpisodio(scTitoloId, slug, stagione, episodio) {
  try {
    ...
    let episodi = [];
    if (typeof data === 'object') {
      episodi = data?.props?.loadedSeason?.episodes || data?.props?.episodes || [];
    } else {
      const match = data.match(/data-page="([^"]+)"/) || data.match(/data-page='([^']+)'/);
      if (match) {
        const json = JSON.parse(match[1].replace(/&quot;/g, '"').replace(/&#039;/g, "'"));
        episodi = json?.props?.loadedSeason?.episodes || json?.props?.episodes || [];
      }
    }
...
async function getStream(videoId) {
  try {
    const { data: htmlWatch } = await axios.get(`${SC_DOMAIN}/watch/${videoId}`, {
      headers: getHeaders(),
      timeout: 15000
    });
    ...
    if (!iframeMatch) { console.warn('[⚠️] Iframe non trovato'); return null; }

    const embedUrl = iframeMatch[1];
    console.log('[🎬 Embed]', embedUrl);

    const { data: htmlEmbed } = await axios.get(embedUrl, {
      headers: { 'User-Agent': SC_USERAGENT, 'Referer': SC_DOMAIN, 'Origin': SC_DOMAIN },
      timeout: 15000
    });
    ...
    if (m3u8) { console.log('[✅ m3u8]', m3u8[1]); return m3u8[1]; }

    console.warn('[⚠️] Nessun link stream');
    return null;
  } catch (e) {
...
builder.defineStreamHandler(async ({ type, id }) => {
  ...
  try {
    ...
    if (!titoli) { console.warn('[⚠️] TMDB fail'); return { streams: [] }; }
    ...
    if (!risultati.length) { console.warn('[⚠️] Nessun risultato SC'); return { streams: [] }; }
    ...
    if (!videoId) { console.warn('[⚠️] video_id non trovato'); return { streams: [] }; }

    const streamUrl = await getStream(videoId);
    if (streamUrl) {
      streams.push({ url: streamUrl, title: `StreamingCommunity\n${titolo.name}`, behaviorHints: { notWebReady: false } });
...
// ─── SERVER — avvia PRIMA, poi inizializza in background ─────────────────────
const PORT = process.env.PORT || 7000;
serveHTTP(builder.getInterface(), { port: PORT });
console.log(`\n🚀 Avviato sulla porta ${PORT}`);

// Inizializzazione asincrona in background — non blocca il server
aggiornaDominio()
